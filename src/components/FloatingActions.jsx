...
function toWhatsAppUrl(number) {
  ...
  const international = digits.startsWith('0') ? `381${digits.slice(1)}` : digits
  const message = encodeURIComponent('Zdravo! Interesuje me vaša ponuda.')
  return `https://wa.me/${international}?text=${message}`
}

export default function FloatingActions() {
  const phone = contact.phones[0].replace(/\s/g, '')
  const whatsappUrl = toWhatsAppUrl(contact.whatsapp)
  ...
  return (
    <div className="floating-actions">
      ...
      <a
        ...
        aria-label={`Pozovi ${contact.phones[0]}`}
        title={contact.phones[0]}
      >
        <span className="floating-actions__icon">
          <PhoneIcon />
        </span>
        <span className="floating-actions__label">Pozovi</span>
      </a>
    </div>
  )
}
